...
<div class="card">
        <h1>Admin Signin</h1>
        <form action="adminsignin.php" method="post"  enctype="multipart/form-data">
        <img src="https://cdn-icons-png.flaticon.com/512/6799/6799093.png" alt="">
        <div class="name">
        <i class="fa-solid fa-file-signature fa-beat-fade"></i>
          <input type="text" name="name" id="formName" placeholder="Enter Admin Name">
        </div>
        <div class="email">
...
if(move_uploaded_file($_FILES["files"]["tmp_name"], $targetFilePath)){
    ...
    $checkAdmin = mysqli_query($conn,"SELECT * FROM `admins` where name='$name' 
    OR email='$email' OR phonenumber='$phonenumber' OR password='$pass'" );
    if(mysqli_fetch_row($checkAdmin)<1){
        $insert = mysqli_query($conn,"INSERT INTO admins(name,email,phonenumber,password,profile)
